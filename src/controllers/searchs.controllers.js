...
const { Op } = require('sequelize');
          ...
          try {
            ...
            const airports = await Airport.findAll({
              ...
              if(airports.length === 0){
                ...
                return res.status(404).send({massage:'There are no airports with this name'})
    ...
    const getFlightsByQuery = async (req, res) => {
      ...
      if(
        !roundTrip || 
        !origin || typeof origin !== 'string' || origin.length < 2 ||
        !destiny || typeof destiny !== 'string'|| destiny.length < 2 ||
        !dateTimeDeparture  ||
        !dateTimeReturn  ||
        !passengers || typeof passengers !== 'number'
        ){
          ...
          if(roundTrip === "false"){
            ...
            try {
                if(origin && destiny && passengers){
                  ...
                  const normalFlights = await Flight.findAll({
                    ...
                    return res.status(200).send(normalFlights)

                }else{
                  
                  return res.status(404).send({message: "Datos no encontrados"})
                }
            } catch (error) {
            ...
            return res.status(200).send(roundTripFlights)
          }
        }
